 ...
 */
var
  _setImmediate = global.setImmediate || process.nextTick,
  util = require('util'),
...
AbstractClass.update = function update(params, cb){
  if (stillConnecting(this.schema, this, arguments, update)) {
    return;
  }
  var Model = this;
...
AbstractClass._forDB = function (data){
  ...
  Object.keys(data).forEach(curry(function (propName){
    var val;
 ...
 */
AbstractClass.create = function create(data, callback){
  if (stillConnecting(this.schema, this, arguments, create)) {
    return;
  }
...
function stillConnecting(schema, context, args, method){
  if (schema.connected) {
    return false;
  }

  schema.once('connected', function (){
    method.apply(context, Array.prototype.slice.call(args));
  });
...
AbstractClass.upsert = AbstractClass.updateOrCreate = function upsert(data, callback){
  if (stillConnecting(this.schema, this, arguments, upsert)) {
    return;
  }
...
AbstractClass.exists = function exists(id, cb){
  if (stillConnecting(this.schema, this, arguments, exists)) {
    return;
  }
...
AbstractClass.find = function find(id, cb){
  if (stillConnecting(this.schema, this, arguments, find)) {
    return;
  }

  var Model = this;

  this.schema.adapter.find(this.modelName, id, curry(function (err, data){
    var obj = null;
    if (data) {
 ...
 * - skip: Number
 *
 * @param {Function} cb (required) called with arguments:
 *
 * - err (null or Error)
 * - Array of instances
 */
AbstractClass.all = function all(params, cb){
  if (stillConnecting(this.schema, this, arguments, all)) {
    return;
  }
