            ...
            obj.trigger('save', function (saveDone){
              obj._adapter().create(modelName, Model._forDB(obj.toObject(true)), function adapterCreate(err, id, rev){
                ...
                if (err) {
                  ...
                  saveDone.call(obj, function saveDoneCall(err){
                    if (err) {
                      d.reject(err);
                    }
                    else
                      createDone.call(obj, function createDoneCall(err){
                      if (err)
                        d.reject(err);
                      else
                        d.resolve(obj);
          ...
          inst.trigger('update', function (updateDone){
            inst._adapter().save(modelName, inst.constructor._forDB(data), function (err){
              if (err) {
                ...
                updateDone.call(inst, function (err){
                  if (err)
                    d.reject(err);
                  else
                    saveDone.call(inst, function (err){
                      if (err)
                        d.reject(err);
                      else
                        d.resolve(inst);
...
AbstractClass.prototype.updateAttributes = function updateAttributes(data, cb){
  ...
  return stillConnecting(Model.schema).then(function(){
    ...
    inst.isValid(data).done(function(valid){
      if (!valid)
          return d.reject(new ValidationError(inst));
      inst.trigger('save', function (saveDone){
        inst.trigger('update', function (done){
          ...
          inst._adapter().updateAttributes(modelName, inst.id, inst.constructor._forDB(inst.toObject(true)), function (err){
            ...
            // *** XXX (SC): now propagating error in after hooks, 
            // and if there is an error, skip after hooks.
            done.call(inst, function (err){
              if (err)
                  d.reject(err);
              else
                saveDone.call(inst, function (err){
